class Event < ApplicationRecord
  ...
  validates :start_date, 
	     presence: {message:"La date de départ doit être renseignée"}
  validate :is_gone?
  validates :duration, 
	     presence: {message:"La durée de l'évênement doit être renseignée"},
	     numericality: {greater_than: 0}
  validate :multiple_of_5?
  validates :title,
	     presence: {message: "Le titre de l'évênement doit être présent"},
	     length: {minimum: 5, maximum: 140, message: "Le titre doit être compris entre 5 et 140 caractères"}
  validates :description,
	     presence: {message: "La description doit être renseignée"},
	     length: {minimum: 20,maximum: 1000, message: "Le titre doit être compris entre 20 et 1000 caractères"}
  validates :price,
	     presence: {message: "le prix doit être renseignée"},
  ...
  validates :location,
	     presence: {message: "Le lieu de l'évênement doit être renseigné"}
